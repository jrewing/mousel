...
describe("game reducer", () => {
  ...
  async function setSmallBlinds() {
    ...
    for (let i = 0; i < state.numberOfPlayers; i++) {
      if (!state.players[i].isDealer) {
        state = await dispatchAndGetState(
          addWager({ player: state.players[i], amount: 1 }),
        );
      }
    }
    return state;
  ...
  it("should handle initial game setup", () => {
    ...
    expect(
      state.deck.filter((c) => !c.isDealt && !c.isDiscarded && !c.isPlayed)
        .length,
    ).toEqual(40); // assuming a standard deck
  ...
  it("should handle dealing cards", async () => {
    ...
    for (let i = 0; i < state.numberOfPlayers; i++) {
      if (!state.players[i].isDealer) {
        state = await dispatchAndGetState(
          addWager({ player: state.players[i], amount: 10 }),
        );
      }
    }
    state = await dispatchAndGetState(dealCards());
  ...
  it("should handle dealing cards when there are already 2 cards dealt", async () => {
    ...
    expect(
      state.deck.filter((c) => !c.isDealt && !c.isDiscarded).length,
    ).toEqual(20);
  ...
  it("should play the first card in a round", async () => {
    ...
    const firstCardPlayed = state.deck.find(
      (c) => c.id === state.players[2].hand[0],
    );
    state = await dispatchAndGetState(
      playCard({
        playerId: state.players[2].id,
        cardId: firstCardPlayed?.id ?? 0,
      }),
    );
    expect(state.rounds[state.currentRound].turns.length).toEqual(1);
    //expect lead suit to be the suit of the card played
    expect(state.rounds[state.currentRound].turns[0].suit).toEqual(
      firstCardPlayed?.suit,
    );
  });
  it("should play all the cards (5 cards equal to number of players) in a round and announce the winner", async () => {
    ...
    const whoCanTakeTrump = selectPlayerWhoCanTakeTrump({
      game: state,
    }) as Player; // Add type assertion
    ...
    expect(
      state.rounds[state.currentRound].turns[0].cardsPlayed.length,
    ).toEqual(5);
    ...
    const winnerPlayer = state.players.find(
      (player) =>
        player.id === state.rounds[state.currentRound].turns[0].winner,
    );
  ...
  it("should the 2 first players refuse trump, 3. folds and 4. takes the trump and 5. cant take trump", async () => {
    ...
    const playerWhoRefusedTrump = state.players.find(
      (player) => player.id === whoCanTakeTrump?.id,
    );
    ...
    const playerWhoFolded = state.players.find(
      (player) => player.id === whoCanTakeTrump3?.id,
    );
    ...
    const playerWhoTookTrump = state.players.find(
      (player) => player.id === whoCanTakeTrump4?.id,
    );
    ...
    expect(selectCurrentTurn(store.getState())?.nextPlayer).toEqual(
      state.players[4].id,
    );
  ...
  it("should go through 4 turns and end the round dividing the pot to the players and", async () => {
    ...
    for (const player of state.players) {
      const playerWhoCanExchange = selectPlayerWhoShouldExchangeCards(
        store.getState(),
      );
      expect(playerWhoCanExchange).toBeDefined();
      state = await dispatchAndGetState(
        exchangeCards(playerWhoCanExchange as Player),
      );
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const startingPlayer =
        currentTurn?.winner || state.players[whoCanTakeTrump.id].id;
      // Find the index of the startingPlayer
      const startIndex = state.players.findIndex(
        (player) => player.id === startingPlayer,
      );
      ...
      const orderedPlayers = state.players
        .slice(startIndex)
        .concat(state.players.slice(0, startIndex));
      ...
      expect(state.rounds[currentRound].turns[i].cardsPlayed.length).toEqual(
      ...
      //get the winning player from state current turn
      const winnerPlayer = state.players.find(
        (player) => player.id === state.rounds[currentRound].turns[i].winner,
      );
  ...
  it("should go through 4 turns 1 player folds and end the round dividing the pot to the players and", async () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // Find the index of the startingPlayer
      const startIndex = playersInRound.findIndex(
        (player) => player.id === startingPlayer?.id,
      );
      // rearrange players so that the starting player is first
      const orderedPlayers = playersInRound
        .slice(startIndex)
        .concat(playersInRound.slice(0, startIndex));
  ...
  it("should go through 4 turns 1 player folds and end the round dividing the pot to the players. Then continuing and play another round", async () => {
    ...
    expect(newRound.roundState).toEqual("Initial");
    state = await setupAndDealCards();
    const whoCanTakeTrumpRound2 = selectPlayerWhoCanTakeTrump(
      store.getState(),
    ) as Player; // Add type assertion
    ...
    const playerWhoCanFoldRound21 = selectPlayerWhoCanFoldOrStay(
      store.getState(),
    );
    expect(playerWhoCanFoldRound21).toBeDefined();
    state = await dispatchAndGetState(
      playerIsIn(playerWhoCanFoldRound21?.id ?? 0),
    );
    const playerWhoCanFoldRound22 = selectPlayerWhoCanFoldOrStay(
      store.getState(),
    );
    expect(playerWhoCanFoldRound22).toBeDefined();
    state = await dispatchAndGetState(
      playerFolds(playerWhoCanFoldRound22?.id ?? 0),
    );
    const playerWhoCanFoldRound23 = selectPlayerWhoCanFoldOrStay(
      store.getState(),
    );
    expect(playerWhoCanFoldRound23).toBeDefined();
    state = await dispatchAndGetState(
      playerIsIn(playerWhoCanFoldRound23?.id ?? 0),
    );
    const playerWhoCanFoldRound24 = selectPlayerWhoCanFoldOrStay(
      store.getState(),
    );
    expect(playerWhoCanFoldRound24).toBeDefined();
    state = await dispatchAndGetState(
      playerIsIn(playerWhoCanFoldRound24?.id ?? 0),
    );
    const playerWhoCanFoldRound25 = selectPlayerWhoCanFoldOrStay(
      store.getState(),
    );
    ...
    const playersInRoundRound2 = state.players.filter(
    ...
    for (let i = 0; i < 4; i++) {
      ...
      expect(startingPlayer?.hasFolded).toBeFalsy();
      // Find the index of the startingPlayer
      const startIndex = playersInRoundRound2.findIndex(
        (player) => player.id === startingPlayer?.id,
      );
      // rearrange players so that the starting player is first
      const orderedPlayers = playersInRoundRound2
        .slice(startIndex)
        .concat(playersInRoundRound2.slice(0, startIndex));
      for (const player of orderedPlayers) {
        //find card that is playable
        let aCardPlayable = false;
        for (const cardId of player.hand) {
      ...
      // Check the state after each full round of turns
      expect(
        state.rounds[currentRoundRound2].turns[i].cardsPlayed.length,
      ).toEqual(playersInRoundRound2.length);
      //expect current turn winner to be defined
      ...
      const winnerPlayer = state.players.find(
        (player) =>
          player.id === state.rounds[currentRoundRound2].turns[i].winner,
      );
  ...
  it("should go through rounds until game over", async () => {
    let state = await dispatchAndGetState(initializeGame(numberOfPlayers));
    ...
    for (let round = 0; round < roundsToPlay; round++) {
      ...
      for (let i = 0; i < state.players.length; i++) {
        const playerWhoCanFoldOrStay = selectPlayerWhoCanFoldOrStay(
          store.getState(),
        );
        if (playerWhoCanFoldOrStay) {
          const action =
            i % 2 === 0
              ? playerIsIn(playerWhoCanFoldOrStay.id)
              : playerFolds(playerWhoCanFoldOrStay.id);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        expect(orderedPlayers[0].id).toBe(
          state.rounds.at(-1)?.turns.at(-1)?.nextPlayer,
        );

        // Log the order of players to confirm correct order
        console.log(
          `Order of players: ${orderedPlayers.map((p) => p.id).join(", ")}`,
        );
        ...
        for (const player of orderedPlayers) {
          ...
          const currentTurn = selectCurrentTurn(store.getState());
          console.log(
            `Next player in isCardPlayable: ${currentTurn?.nextPlayer}`,
            startingPlayerId,
          );
          console.log(
            `Order of players: ${orderedPlayers.map((p) => p.id).join(", ")}`,
      ...
      expect(state.rounds[currentRound].turns.length).toEqual(4);
      expect(
        state.rounds[currentRound].roundState === "RoundOver" ||
          state.rounds[currentRound].roundState === "GameOver",
      ).toBeTruthy();
  ...
  it.only('should go through rounds where everyone refuses trump and decides to "flip" or not. On flip, a new card is used to find a different trump suit. If no flip, the round is over. ', async () => {
    let state = await dispatchAndGetState(initializeGame(numberOfPlayers));

    const roundsToPlay = 1; // Number of rounds to test
    for (let round = 0; round < roundsToPlay; round++) {
      console.log(`Round ${round + 1}`);
      state = await setupAndDealCards();
      for (const player of state.players) {
      ...
      const trumpSuit = selectTrumpSuit(store.getState());

      //expect all players to have refused trump
      expect(state.players.every((p) => p.hasRefusedTrump)).toEqual(true);
      const whoCanFlip = selectPlayerWhoCanFlipTrump(
        store.getState(),
      ) as Player;
      expect(whoCanFlip).toBeDefined();
      state = await dispatchAndGetState(refuseToFlipTrump(whoCanFlip.id));
      const whoCanFlip2 = selectPlayerWhoCanFlipTrump(
        store.getState(),
      ) as Player;
      expect(whoCanFlip2).toBeDefined();
      expect(whoCanFlip2.id).not.toEqual(whoCanFlip.id);
      state = await dispatchAndGetState(flipTrump(whoCanFlip2.id));

      const newTrumpSuit = selectTrumpSuit(store.getState());
      expect(newTrumpSuit).toBeDefined();
      expect(newTrumpSuit).not.toEqual(trumpSuit);
